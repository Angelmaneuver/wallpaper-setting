...
export class File {
	...
	private static getChildren(
		...
		files       = dirents.filter(
			dirent => dirent.isFile() && (filters.includes(File.getExtension(dirent.name)) || !filtering)
			).map(
				( {name} ) => fullPath ? path.join(targetPath, name) : name
			);
			
		result = result.concat(files);

		if(recursive) {
			let directories = dirents.filter(dirent => dirent.isDirectory()).map(({ name }) => path.join(targetPath, name));
